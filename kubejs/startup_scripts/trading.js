...
onEvent("item.registry", event => {
  ...
  tradeList().forEach(trade => {
    ...
    event.create('trade_card_' + trade[3]).texture(modpackId + ":/item/trading/trade_card").displayName('Trade Card').tooltip(isTag + trade[2] + ':' + trade[3])
    if (professions.includes(trade[0]) != true) {
      professions.push(trade[0])
    }
  })
  professions.forEach(profession => {
    event.create('profession_card_' + profession).texture(modpackId + ":/item/trading/profession_card").displayName('Profession Card').tooltip(modpackId + ':' + profession)
  })
})