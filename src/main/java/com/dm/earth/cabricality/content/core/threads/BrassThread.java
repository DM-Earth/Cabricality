...
public class BrassThread implements TechThread {
	...
	public void addRecipes(AddRecipesCallback.RecipeHandler handler) {
		...
		registerCrystalProcess(handler, AE2.id("certus_quartz_crystal"), AE2.id("certus_crystal_seed"),
				AE2.id("fluix_dust"), MC.id("water"));
		registerCrystalProcess(handler, AE2.id("fluix_crystal"), AE2.id("fluix_crystal_seed"), AE2.id("fluix_dust"),
				CABF.id("waste"));

		handler.register(recipeId("mixing", "sky_stone"), id -> new MixingRecipe(
				new FreePRP(id).setFluidIngredient(FluidIngredient.fromFluid(Fluids.WATER, FluidConstants.BOTTLE))
						.setIngredient(ListUtil.loop(Ingredient.ofItems(AE2.asItem("sky_dust")), 3))
						.setFluidResult(new FluidStack(FluidVariant.of(CabfFluids.SKY_STONE), FluidConstants.BOTTLE))));

		handler.register(recipeId("mixing", "redstone"), id -> new MixingRecipe(
				new FreePRP(id)
						...
						.setFluidResult(new FluidStack(FluidVariant.of(CabfFluids.REDSTONE), FluidConstants.INGOT * 2))
						.setResult(new ProcessingOutput(AE2.asItem("certus_quartz_crystal").getDefaultStack(), 1))));

		handler.register(recipeId("mixing", "polished_rose_quartz"), id -> new MixingRecipe(new FreePRP(id)
		...
		handler.register(recipeId("filling", "electron_tube"), id -> new FillingRecipe(
				new FreePRP(id).setIngredient(Ingredient.ofItems(CR.asItem("polished_rose_quartz"))).setFluidIngredient(
						FluidIngredient.fromFluid(TC.asFluid("molten_iron"), FluidConstants.NUGGET))
	...
	private void registerCrystalProcess(AddRecipesCallback.RecipeHandler handler, Identifier crystal, Identifier seed,
			Identifier dust, Identifier fluid) {
		...
		handler.register(recipeId("milling", crystal.getPath()),
				id -> new MillingRecipe(new FreePRP(id)
						.setIngredient(Ingredient.ofItems(crystalItem))
