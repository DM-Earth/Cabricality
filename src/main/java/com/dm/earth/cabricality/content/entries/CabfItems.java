...
public class CabfItems {
	...
	public static final Item RADIANT_SHEET = registerItemModeled("radiant_sheet", new GlintedItem(Properties.DEFAULT),
	...
	public static void register() {
		...
		Arrays.stream(Professions.values()).forEach(professionEntry -> {
			...
			profession.entries().forEach(
					entry -> registerItemModeled("trade_card_" + entry.hashString(), new TradeCardItem(Properties.CARD),
		...
		Arrays.stream(TradingEntry.CoinTypes.values()).forEach(coinType -> {
			...
			registerItemModeled(coinType.getId().getPath() + "_top", new Item(Properties.CARD),
					ItemModelGenerator.parented(Cabricality.id("item", "coin", coinType.getId().getPath() + "_top").toString()));
			registerItemModeled(coinType.getId().getPath() + "_bottom", new Item(Properties.CARD),
					ItemModelGenerator.parented(Cabricality.id("item", "coin", coinType.getId().getPath() + "_bottom").toString()));
		});
