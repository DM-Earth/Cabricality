...
public class RecipeBuilderUtil {
	...
	public static JsonObject generateMelting(Identifier input, Identifier fluid, long amount,
		...
		json.add("ingredient", JRecipeUtil.itemEntry(input));
		json.add("result", JRecipeUtil.fluidEntry(fluid, amount));
		json.addProperty("temperature", temperature);
		json.addProperty("time", time);
		if (byProduct != null) {
			JsonArray byproducts = new JsonArray();
			byproducts.add(JRecipeUtil.fluidEntry(byProduct, byAmount));
