...
public class Alchemist {
	...
	public static void processChaoticRecipe(@NotNull HopperMinecartEntity minecart,
		...
		if (catalysts.isEmpty()) {
			...
			if (catalystTargetTemp != null) {
				...
				addSlots.put(
						tempList.get(0),
						new ItemStack(Registries.ITEM.get(Cabricality.id("catalyst_jar_" + catalystTargetTemp.hashString())))
				);
				success = true;
			}

			else if (!reagentsList.isEmpty() && catalysts.isEmpty()) {
				// Reagents -> Dusts
				Catalyst targetCatalyst = Objects.requireNonNull(Reagents.get(reagentsList.get(0))).getCatalyst();

				boolean canContinue = true;
				for (Reagent reagent : reagentsList)
					if (Objects.requireNonNull(Reagents.get(reagent)).getCatalyst() != targetCatalyst) canContinue = false;
				...
				if (canContinue) {
					...
					for (Reagent reagent : expected) {
						if (reagentsList.contains(reagent)) {
							if (reagentsList.indexOf(reagent) == expected.indexOf(reagent)) correct++;
							else wrongPos++;
						}
					}

					ArrayList<Integer> tempList = new ArrayList<>(reagents.keySet());
					tempList.sort(null);
		...
		if (!success && reagents.isEmpty() && catalysts.size() > 1) {
			...
			if (existed.equals(expected)) {
				addSlots.put(
						tempList.get(0),
						new ItemStack(Registries.ITEM.get(Cabricality.id("catalyst_jar_" + CHAOTIC_CATALYST.hashString())))
				);
				success = true;
			}
				...
				for (Catalyst catalyst : expected)
					if (existed.contains(catalyst)) {
						if (existed.indexOf(catalyst) == expected.indexOf(catalyst)) correct++;
						else wrongPos++;
					}
			...
			if (canContinue) {
				...
				for (Map.Entry<Reagent, Reagent> entry : map.entrySet()) {
					if (entry.getValue() == reagent) {
						addSlots.put(
								tempList.get(0),
								new ItemStack(Registries.ITEM.get(Cabricality.id("reagent_jar_" + entry.getKey().hashString())))
						);
						break;
					}
				}
	...
	private static <T> @NotNull ArrayList<T> mapToList(@NotNull Map<Integer, T> map) {
		return Arrays.stream((new ArrayList<>(map.keySet())).toArray(new Integer[0]))
				.sorted()
	...
	public static Map<Reagent, Reagent> possibleSpecialReagentChaoticMap(ServerWorld world) {
		ArrayList<Reagent> reagents = Arrays.stream(Reagents.values())
	...
	@Nullable
	private static Catalyst getMatchedCatalyst(ArrayList<Reagent> existed, @NotNull Map<Catalyst, ArrayList<Reagent>> map) {
		return map.entrySet().stream()
				.filter(entry -> entry.getValue().equals(existed))
				.findFirst()
				.map(Map.Entry::getKey)
				.orElse(null);
	}

	public static @NotNull Map<Catalyst, ArrayList<Reagent>> possibleReagentMap(ServerWorld world) {
		return Arrays.stream(Reagents.values())
				.filter(Reagents::isLinked)
